...
if(submitcheck('connectsubmit') and $pluginop == 'reset') {
    $newusername = sqq_checkusername($_GET['newusername']);
    ...
    C::t('#clogin_qq#clogin_member_qqconnect')->update($connect_member['uid'], array('isreset' => 1, 'isregister' => 0));


    showmessage('clogin_qq:connect_config_success',dreferer());
}else if(submitcheck('connectsubmit') and $pluginop == 'unbind'){

    if($connect_member['isreset'] == 0){
        showmessage('clogin_qq:connect_bind_need_password');
    }
    if($connect_member['isregister'] == 1){
        ...
        C::t('#clogin_qq#clogin_member_qqconnect')->delete($_G['uid']);
    ...
    showmessage('clogin_qq:connect_config_unbind_success',dreferer());
    ...
    if(defined('IN_MOBILE')) {
        global $_G, $sqqconnect;
        include_once template('clogin_qq:bind');
        exit();
    }
