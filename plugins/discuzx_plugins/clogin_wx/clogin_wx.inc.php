...
$connect_member = C::t('#clogin_wx#clogin_member_wxconnect')->fetch_fields_by_openid($openid);

if($_G['uid']) { //已登录状态，直接绑定QQ号

    if($connect_member && $connect_member['uid'] != $_G['uid']) {
        showmessage('clogin_wx:connect_register_bind_already', $referer);
    }

    $connect_member = C::t('#clogin_wx#clogin_member_wxconnect')->fetch($_G['uid']);
    if($connect_member){
        C::t('#clogin_wx#clogin_member_wxconnect')->update($_G['uid'], array(
            'openid' => $openid,
            'token' => $access_token,
    ...
    $referer = $_G['siteurl'] . 'home.php?mod=spacecp&ac=plugin&id=clogin_wx:bind';
    showmessage('clogin_wx:connect_register_bind_success', $referer);

}else{ // 未登录状态
    if($connect_member) { //账号已存在，直接登录进论坛
        C::t('#clogin_wx#clogin_member_wxconnect')->update($connect_member['uid'], array(
            'token' => $access_token,
            'nickname' => $userinfo['nickname'],
            'faceimg' => $userinfo['faceimg'],
            'location' => $userinfo['location'],
        ...
        $params['mod'] = 'login';
        swxconnect_login($connect_member['uid']);
    ...
    } elseif($swxconnect['on_autoregister']) { //账号不存在，并且开启了自动注册用户
        ...
        loaducenter();
        $groupid = $swxconnect['on_gr'] ? $swxconnect['on_gr'] : $_G['setting']['newusergroupid'];
        ...
        $username = swx_user_checkname($username);
        ...
        dsetcookie('swxconnect_openid', $openid, 31536000);
        dsetcookie('swxconnect_access_token', $access_token, 31536000);
        dsetcookie('swxconnect_guestinfo_nickname', $userinfo["nickname"], 31536000);
        dsetcookie('swxconnect_guestinfo_faceimg', $userinfo['faceimg'], 31536000);
        dsetcookie('swxconnect_guestinfo_location', $userinfo['location'], 31536000);
        dsetcookie('swxconnect_guestinfo_gender', $userinfo["gender"], 31536000);
    
        $referer = $_G['siteurl'] . 'member.php?mod='.$_G['setting']['regname'].'&sqcaction=swxconnect&referer=' . urlencode($referer);
