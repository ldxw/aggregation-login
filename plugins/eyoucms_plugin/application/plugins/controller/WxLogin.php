...
use weapp\WxLogin\vendor\Oauth;
use think\Db;
...
class WxLogin extends Base
{
    ...
    public function __construct(){
        parent::__construct();
        $config = Db::name('weapp')->where('code', 'WxLogin')->find();
        if (empty($config['status'])) {
            $this->error('请后台启用微信快捷登录插件！');
        }
        $data = unserialize($config['data']);
        if (empty($data['appurl']) || empty($data['appid']) || empty($data['appkey'])){
            $this->error("登录失败，请联系管理员配置微信快捷登录信息");
        }
        $callback = $this->request->domain().$this->root_dir.'/index.php?m=plugins&c=WxLogin&a=callback&lang='.$this->home_lang;
    ...
    public function callback(){
        ...
        if(isset($arr['code']) && $arr['code']==0){
            ...
            $connect = Db::name("weapp_wxlogin")->where('openid', $openid)->find();    //判断绑定关系是否存在
            if($connect){ //绑定关系存在
                ...
                if (empty($users)) { //用户不存在，自动注册账号，并且登陆
                    ...
                    if (!empty($users['users_id'])) {
                        Db::name('weapp_wxlogin')->where('id', $connect['id'])->update([
    ...
    {
        if (empty($username)) {
            $username = 'WX'.get_rand_str(8,0,1);
        }
