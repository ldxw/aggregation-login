...
namespace weapp\Wblogin\controller;

use think\Db;
use app\common\controller\Weapp;
use weapp\Wblogin\model\WbloginModel;

/**
 * 插件的控制器
 */
class Wblogin extends Weapp
{
    /**
     * 实例化模型
     */
    ...
    public function afterInstall()
    {
        ...
        Db::name('weapp')->where(['code'=>'Wblogin'])->update($savedata);
    }
    ...
    public function index()
    {
        ...
        $qqlogin_url = $this->root_dir.'/index.php?m=plugins&c=Wblogin&a=login';
    ...
    public function save()
    {
        if (IS_POST) {
            ...
            $r = Db::name('weapp')->where(array('code' => 'Wblogin'))->update($saveData);
            if ($r) {
                adminLog('编辑' . $this->weappInfo['name'] . '成功'); // 写入操作日志
                $this->success("操作成功!", weapp_url('Wblogin/Wblogin/index'));
            }
        }
        $this->error("操作失败");
    }
