<?php
namespace oauth\clogingoogle;

use app\common\lib\Oauth;

class Clogingoogle extends Oauth{
	const LOGIN_TYPE = "google";

	// 插件基础信息
    public $info = array(
        'name'        => 'Clogingoogle', // 必填 插件标识(唯一)
        'title'       => 'Google', // 必填 插件显示名称
        'description' => '彩虹聚合登录-谷歌登录', // 必填 插件功能描述
        'author'      => '彩虹聚合登录', // 必填 插件作者
        'version'     => '1.0.0',  // 必填 插件版本
        'help_url'    => 'https://u.cccyun.cc/', // 选填 申请链接
        'author_url'  => '', // 选填 作者链接
        'logo_url'    => 'google.svg', // 选填 图标地址(可以自定义支付图片地址)
    );
